      ...
      <aside className={`bg-dark-bg text-dark-text-primary flex flex-col flex-shrink-0 absolute md:relative z-40 h-full transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isCollapsed ? 'md:w-20' : 'md:w-64'}`}>
        <div className="flex items-center justify-between p-4 border-b border-dark-border h-[65px] flex-shrink-0">
          <div className="flex-1 flex items-center gap-3 overflow-hidden">
            {isCollapsed ? (
              <button onClick={() => setIsSidebarCollapsed(false)} className="mx-auto" title="Expand sidebar">
          ...
          {!isCollapsed && (
            <button onClick={() => setIsSidebarCollapsed(true)} className="hidden md:block p-1 text-dark-text-secondary hover:text-dark-text-primary" aria-label="Collapse sidebar">
              <CollapseIcon className="w-6 h-6 transition-transform duration-300" />
            </button>
          )}

          <button onClick={() => setSidebarOpen(false)} className={`md:hidden p-1 ${isCollapsed ? 'hidden' : ''}`}>
        ...
        <div className="p-2 border-t border-dark-border flex-shrink-0">
          <button onClick={openSettings} title="Settings" className={`w-full flex items-center gap-3 px-4 py-2 text-sm font-semibold text-dark-text-secondary rounded-md hover:bg-dark-input hover:text-dark-text-primary transition-colors ${isCollapsed ? 'md:justify-center md:px-2' : ''}`}>
